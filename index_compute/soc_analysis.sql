...
DROP table if exists faers.ror_soc_a_b;
CREATE TABLE faers.ror_soc_a_b as
select
       if(t2.primaryid is not null, 'THIS_DRUG', 'OTHER_DRUG') as drug_name
       ,replace(t1.soc,'\\r', '') as soc
...
group by 1,2
;
-- ALTER table rename faers.ror_soc_a_b faers.ror_soc_DE_dE;
-- ALTER table faers.ror_soc_DE_dE  modify drug varchar(255) character set utf8mb3 collate utf8mb3_bin;
-- D： DE+De;  d: dE+dE
DROP table if exists faers.ror_soc_D_d;
CREATE TABLE faers.ror_soc_D_d as
select
       if(t2.primaryid is not null, 'THIS_DRUG', 'OTHER_DRUG') as drug_name
       ,count(distinct t1.primaryid) as primaryid_cnt
from faers.faers_reac t1
...
--
DROP TABLE  if exists faers.drug_soc_stats;
CREATE TABLE faers.drug_soc_stats as
select
     t1.drug_name, t1.soc,
     t1.primaryid_cnt as drug_event_cnt,
     coalesce(t2.primaryid_cnt,0) - t1.primaryid_cnt as drug_otherevent_cnt
from faers.ror_soc_a_b t1
left join faers.ror_soc_D_d t2
on t1.drug_name = t2.drug_name
;

DROP TABLE  if exists faers.ror_soc_abcd;
CREATE TABLE faers.ror_soc_abcd as
select soc,
